...
class FirebaseRegistry:
    ...
    def put_pairing(self, pairing_code: str, payload: dict[str, Any]) -> None:
        requests.put(self._url(f"pairings/{pairing_code}"), json=payload, timeout=5).raise_for_status()

    def fetch_pairing(self, pairing_code: str) -> dict[str, Any] | None:
        response = requests.get(self._url(f"pairings/{pairing_code}"), timeout=5)
